...
public class GenerateOrder extends OneShotBehaviour{
	...
	public void action() {
		...
		orderNo++;
		System.out.println("customer order sending: " + order.getScreen().getDisplaySize() + " " + order.getRam().getRAMSize() + " " + order.getBattery().getBatteryLife() +" " + order.getStorage().getStorageSize());
		...
		try {
		 ...
		 send(enquiry);
		 System.out.println("customer order sent" + order.toString());
		}
