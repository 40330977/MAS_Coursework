				...
				try {
					...
					if(ce instanceof Action) {
						...
						if (action instanceof SupplierOrder) {
							...
							System.out.println("exp test: " + order.getQuantity()+ " " + order.getDueIn());
							recievedOrders.add(order);}
							else {
								System.out.println("exp sup array: " + recievedOrders.toString());
								orderReciever = true;}
			...
			for(SupplierOrder order : recievedOrders) {
				if(order.getDueIn() == day) {
					...
					try {
					 ...
					 send(enquiry);
					 System.out.println("Order Returned: " + order.getDueIn() +" " + order.getOrderID()+" "+order.getQuantity());
					}
					catch (CodecException ce) {
					 ce.printStackTrace();
					}
