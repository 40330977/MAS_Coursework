...
public class ManufacturerAgent extends Agent{
	...
	public class ScheduleManager1 extends OneShotBehaviour{
	...
	public void action() {
		...
		System.out.println(recievedOrdersSched.size()+" "+recievedOrdersSched.toString());
		if(recievedOrders!=null) {
		...
		if(bestOrder.getQuantity()<=dailyprod.get(day+1)) {
			int remprod = dailyprod.get(day+1) - bestOrder.getQuantity();
			dailyprod.set(day + 1, remprod);
			orderSchedule.put(bestOrder, day);
		...
		for(CustomerOrder order : recievedOrders) {
			if(order!=bestOrder) {
				if(order.getQuantity()<=dailyprod.get(day+1)) {
					int remprod = dailyprod.get(day+1) - order.getQuantity();
					dailyprod.set(day + 1, remprod);
					orderSchedule.put(order, day);
	...
	public class Build extends OneShotBehaviour {
		...
		public void action() {
			for(CustomerOrder order: buildSchedule.keySet()) {
				if(buildSchedule.get(order) == day) {
					System.out.println("day scheduled: " + order.getOrderID());
					if(CheckStock(order)) {
						order.setBuiltAndShipped(true);
						System.out.println("parts in stock!");
						System.out.println(order.getOrderID() + " shipped! " + order.isBuiltAndShipped());
					}
					else {
						System.out.println("parts not in stock!");
						System.out.println(order.getOrderID() + "not shipped! " + order.isBuiltAndShipped());
					}
				}
			}
	...
	public boolean CheckStock(CustomerOrder order) {
		...
		if(order.getBattery().getBatteryLife()==2000) {
			if(order.getQuantity()<= warehouse.getBattery2000()) {
				...
				warehouse.setBattery2000(batsub);
		...
		if(bat==true&&ram==true&&screen==true&&store==true) {
			order.setBuiltAndShipped(true);
			System.out.println(order.getOrderID() + " shipping!");
			return true;
		}
		else {return false;}
	}
...
public class EndDay extends OneShotBehaviour {
		...
		public void action() {
			...
			for(CustomerOrder order: buildSchedule.keySet()) {
				...
				if(buildSchedule.get(order) == day-1&&order.isBuiltAndShipped()) {
					netProf += order.getNetProfit();
					System.out.println("Not late warehouse cost: "+order.getOrderID() + " " + order.isBuiltAndShipped());
				}
				if(buildSchedule.get(order) == day-1&&!order.isBuiltAndShipped()) {
					netProf += (order.getNetProfit()-order.getLatePenalty());
					System.out.println("late warehouse cost: "+order.getOrderID() + " " + order.isBuiltAndShipped());
					late++;
				}
			}
			...
			System.out.println(warehouse.getStorage256());
			System.out.println(warehouse.getStorage64());
			System.out.println("Warehouse cost: " + warehouse.StorageCost());
			netProf -= warehouse.StorageCost();

			System.out.println("day over. " + day + " is new day");
			System.out.println("Net Profit is: " + netProf+" Late orders: " +late + " total orders: " + buildSchedule.size());
